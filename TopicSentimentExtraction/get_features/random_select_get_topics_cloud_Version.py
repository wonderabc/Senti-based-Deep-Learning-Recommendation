...
source_path = "../mnt/reviews_generate_topics.txt"
save_path = "../mnt/data/random_select_reviews_50w.txt"
reviews_num = 500000
...
def main():
    ...
    data = get_review(save_path)  # 读取随机评论
    print("\n开始数据清洗……")
    doc_clean = []
    for i in tqdm(range(len(data))):
        doc = data[i]
        doc_clean.append(clean(doc).split())
    dictionary = corpora.Dictionary.load('../mnt/model/reviews_dict_after_filtering_v2.dict')
    # corpus = [dictionary.doc2bow(text) for text in doc_clean]  # corpus里面的存储格式（0, 1), (1, 1), (2, 1), (3, 1), (4, 1), (5, 1), (6, 1)
    # corpora.MmCorpus.serialize('../mnt/model/corpus.mm', corpus)
    corpus = corpora.MmCorpus('../mnt/model/corpus.mm')  # 加载语料库迭代器
    testset = []
    ...
    # num_topics_list = [15, 20, 25, 30, 35, 40, 45, 50]  # 候选主题数
    num_topics_list = [55, 60, 65, 70, 75]
    prep_list = []  # 存储困惑度
    for num_topics in num_topics_list:
        ...
        print("存储LDA模型……")
        savename = "../mnt/model/lda_" + "topicnum=" + str(num_topics) + str(time.strftime('_%m_%d_%H_%M_%S', time.localtime(time.time()))) + ".pkl"
